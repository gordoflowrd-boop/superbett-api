...
// Recibe numero_ticket (ej: BNC-000042)
// 1. Busca el ticket y valida banca
// 2. Verifica que no tenga ganadores pagados
// 3. Llama anular_ticket(uuid) que revierte exposición
// =============================================
router.post('/:numero/anular', async (req, res) => {
  ...
  try {
    // Buscar ticket con info completa + verificar si tiene ganadores
    const check = await query(
    ...
    // Vendedor solo puede anular de su banca
...
router.post('/:numero/pagar', async (req, res) => {
  ...
  try {
    ...
    // Vendedor solo puede pagar de su banca
    if (req.usuario.rol === 'vendedor' && ticket.banca_id !== banca_id) {
      return res.status(403).json({ error: 'Ticket no pertenece a tu banca' });
    }
...
// =============================================
// GET /api/tickets?jornada_id=&banca_id=&fecha=&fecha_desde=&fecha_hasta=
// Listado de tickets [admin/central/rifero/vendedor]
// vendedor solo ve tickets de su banca
// =============================================
router.get('/', async (req, res) => {
  const { jornada_id, banca_id, fecha, fecha_desde, fecha_hasta } = req.query;

  // Vendedor solo puede ver su banca
...
// =============================================
// GET /api/tickets/ventas-lista?fecha=&loteria_id=
// Ventas agrupadas por modalidad/número/lotería
// Para la pantalla "Venta por Lista"
// El vendedor solo ve su banca automáticamente
// =============================================
router.get('/ventas-lista', async (req, res) => {
  ...
  try {
    // Tickets normales (Q, P, T) agrupados por modalidad + número + lotería
    const normales = await query(
      `SELECT
       ...
       ORDER BY td.modalidad, SUM(td.cantidad) DESC, SUM(td.monto) DESC`,
      [banca_id, fechaFiltro, loteria_id || null]
    );

    // Super Palé agrupado por número + combo de loterías
    ...
    // Total general de la banca en esa fecha
    ...
    // Loterias disponibles para el selector (jornadas de esa fecha)
